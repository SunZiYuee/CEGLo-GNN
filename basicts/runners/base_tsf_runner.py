...
def rescale(data, mini, std, mean):
    data = data - mini
    if len(data.shape) == 4:
...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def build_train_dataset(self, cfg: dict):
        ...
        # cfg["DATASET_CLS"] -> PretrainingDataset类
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        # re-scale data
        # [8, 1512, 207] for encoder,  [8, 12, 207, 1] for gnn
        prediction_rescaled = rescale(torch.squeeze(forward_return[0]), self.mini, self.std, self.mean)
        real_value_rescaled = rescale(torch.squeeze(forward_return[1]), self.mini, self.std, self.mean)
    ...
    def test(self):
        ...
        prediction = rescale(prediction, self.mini, self.std, self.mean)
        real_value = rescale(real_value, self.mini, self.std, self.mean)
